#!/usr/bin/env bash
# PostToolUse hook (matcher: Edit|Write) - run a language-appropriate formatter
# on the edited file. Fails open: formatter errors are logged, never block.
#
...
fmt_log="${CLAUDE_PROJECT_DIR:-.}/.claude/hooks/format.log"
